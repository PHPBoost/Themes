        ...
        # IF C_VISIT_COUNTER #
            <div id="visit-countainer" class="hidden-small-screens">
                <div class="visit-counter-total">
                    <span>{@user.guests} : <span class="pinned visitor">{VISIT_COUNTER_TOTAL}</span></span>
                    &nbsp;<span>{@date.today} : <span class="pinned visitor">{VISIT_COUNTER_DAY}</span></span>
                </div>
            </div>
        # ENDIF #
	</div>
	<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="225" id="footer-svg">
		<defs>
			<linearGradient id="grad1" x1="20%" y1="10%" x2="20%" y2="80%">
				<stop offset="0%" style="stop-color:var(--bgc-main);stop-opacity:1" />
				<stop offset="50%" style="stop-color:var(--bgc-bottom);stop-opacity:1" />
				<stop offset="100%" style="stop-color:var(--bgc-alt);stop-opacity:1" />
			</linearGradient>
		</defs>
