		...
		# ENDIF #
			<ul class="connect-elements-container">
				<li class="connect-element connect-profil">
					<a href="${relative_url(UserUrlBuilder::home_profile())}" class="small" title="{@dashboard}">
						<i class="fa fa-profil"></i><span class="clear-name">{@dashboard}</span>
					</a>
				</li>
				<li class="connect-element connect-pm# IF C_HAS_PM # connect-event# ENDIF #">
					<a href="{U_USER_PM}" class="small" title="{L_PM_PANEL}">
						<i class="fa fa-envelope# IF C_HAS_PM # blink# ENDIF #"></i><span class="clear-name">{L_PM_PANEL}</span># IF C_HAS_PM #<span class="blink">({NUMBER_PM})</span># ENDIF #
					</a>
				</li>
				# IF C_ADMIN_AUTH #
				<li class="connect-element connect-admin# IF C_UNREAD_ALERT # connect-event# ENDIF #">
					<a href="${relative_url(UserUrlBuilder::administration())}" class="small" title="{L_ADMIN_PANEL}">
						<i class="fa fa-wrench# IF C_UNREAD_ALERT # blink# ENDIF #"></i><span class="clear-name">{L_ADMIN_PANEL}</span># IF C_UNREAD_ALERT #<span class="blink">({NUMBER_UNREAD_ALERTS})</span># ENDIF #
					</a>
				</li>
				# ENDIF #
				# IF C_MODERATOR_AUTH #
				<li class="connect-element connect-modo">
					<a href="${relative_url(UserUrlBuilder::moderation_panel())}" class="small" title="{L_MODO_PANEL}">
						<i class="fa fa-legal"></i><span class="clear-name">{L_MODO_PANEL}</span>
					</a>
				</li>
				# ENDIF #
				<li class="connect-element connect-contribution# IF C_UNREAD_CONTRIBUTION # connect-event# ENDIF #">
					<a href="${relative_url(UserUrlBuilder::contribution_panel())}" class="small" title="{L_CONTRIBUTION_PANEL}">
						<i class="fa fa-file-text# IF C_UNREAD_CONTRIBUTION # blink# ENDIF #"></i><span class="clear-name">{L_CONTRIBUTION_PANEL}</span># IF C_UNREAD_CONTRIBUTION #<span class="blink">({NUMBER_UNREAD_CONTRIBUTIONS})</span># ENDIF #
					</a>
				</li>
				<li class="connect-element connect-disconnect">
					<a href="${relative_url(UserUrlBuilder::disconnect())}" class="small" title="{@disconnect}">
						<i class="fa fa-sign-out"></i><span class="clear-name">{@disconnect}</span>
					</a>
				</li>
			</ul>
		</div>
