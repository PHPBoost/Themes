...
# ELSE #
	...
	<div class="connect_align">
		<form action="{U_CONNECT}" method="post" onsubmit="return check_connect();" class="connect_align">
			...
			<input type="hidden" name="redirect" value="{REWRITED_SCRIPT}" />
			<input type="submit" name="connect" style="margin-top:8px;" value="{L_CONNECT}" class="submit" /></p>
		</form>
		# IF C_USER_REGISTER #
		<form action="{U_REGISTER}" method="post" class="connect_align">
			<p class="connect_inline"><input type="submit" name="register" style="margin-top:8px;" value="{L_REGISTER}" class="submit" /></p>
		</form>
		# ENDIF #
	</div>
	# ELSE #
